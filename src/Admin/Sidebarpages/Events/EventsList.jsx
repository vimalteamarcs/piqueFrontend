...
const EventsList = () => {
  ...
  const [search, setSearch] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState(search); // Added debouncedSearch state
  const [flag, setFlag] = useState(false);

  // Added a Debouncing function
  useEffect(() => {
    const handler = setTimeout(() => {
      setDebouncedSearch(search); // Set the debounced search
    }, 500); // Adjust delay as needed

    return () => {
      clearTimeout(handler); // Clean up the timeout on each change
    };
  }, [search]);

  useEffect(() => {
    fetchEvents(pagination.current, pagination.pageSize, debouncedSearch); // Use debouncedSearch for fetching events
    console.log(events);
  }, [pagination.current, pagination.pageSize, debouncedSearch, flag]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status) => {
        ...
        return (
          <span className={`badge text-uppercase text-black ${badgeClass}`}>
            {status}
          </span>
        );
      },
