...
export default function OtpVerifyModal({ email, onSuccess, onClose }) {
  ...
  const [verified, setVerified] = useState(false);
  const [resending, setResending] = useState(false);
  const [error, setError] = useState(""); // Error state for OTP validation
  ...
  const handleVerifyOtp = async (e) => {
    ...
    if (!otp) {
      // Show an error if OTP is not filled
    ...
    try {
      ...
      const errorMessage =
        ...
        error.response?.data?.error ||   // Sometimes backend uses 'error' field
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      toast.error("Failed to resend OTP. Please try again later.", { position: "top-center" });
    } finally {
      setResending(false);
    }
  };
        ...
        <div className="modal-dialog modal-dialog-centered" role="document">
          <div className="modal-content p-4">
            ...
            <div className="modal-body">
              ...
              <div className="d-grid gap-2 mt-4">
                <button className="btn btn-success rounded-3" onClick={handleVerifyOtp}>
                  Verify OTP
                </button>
